-- Run this in Supabase SQL Editor: https://supabase.com/dashboard/project/gexdoytyemjzonxguvhv/sql
...
-- Create policy
DROP POLICY IF EXISTS "Anyone can view active government schemes" ON public.government_schemes;
...
-- Create policy
DROP POLICY IF EXISTS "Users can manage their own applications" ON public.scheme_applications;
CREATE POLICY "Users can manage their own applications" ON public.scheme_applications
  FOR ALL USING (auth.uid() = user_id);

-- Insert sample schemes (delete existing first to avoid duplicates)
